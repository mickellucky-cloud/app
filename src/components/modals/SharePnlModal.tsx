...
export const SharePnlModal: React.FC<SharePnlModalProps> = ({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 bg-black/80 dark:bg-black/90 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
        <motion.div
          initial={{ opacity: 0, scale: 0.92, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.92, y: 20 }}
          transition={{ type: 'spring', stiffness: 350, damping: 28 }}
          className="relative w-full max-w-md bg-[#0A0D14] border border-white/[0.12] rounded-3xl p-5 sm:p-6 text-white shadow-[0_20px_60px_rgba(0,0,0,0.8)] overflow-hidden"
        >
          ...
          <div className="relative z-10 flex items-center justify-between pb-3 border-b border-white/10 mb-4">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-purple-500/20 border border-purple-500/40 text-purple-400 flex items-center justify-center">
                <Share2 className="w-3.5 h-3.5" />
              </div>
              <h3 className="font-bold text-sm tracking-wide text-white">Share Portfolio Performance</h3>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="relative z-10 grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-white/10 hover:bg-white/15 active:scale-95 border border-white/15 text-xs font-bold text-white transition-all shadow-xs"
            >
              {copied ? (
                <>
