...
export const AiTraderModal: React.FC<AiTraderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 dark:bg-black/80 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fadeIn">
      <div className="w-full max-w-md bg-white dark:bg-[#0F1320] border-t sm:border border-slate-200 dark:border-purple-500/30 rounded-t-3xl sm:rounded-3xl p-5 safe-area-bottom max-h-[85vh] overflow-y-auto animate-slideUp text-slate-900 dark:text-slate-100 shadow-2xl transition-colors">
        ...
        {isActive ? (
          <div className="py-8 text-center space-y-3">
            <div className="w-14 h-14 rounded-2xl bg-emerald-100 dark:bg-emerald-500/20 border border-emerald-300 dark:border-emerald-500/40 flex items-center justify-center text-emerald-600 dark:text-emerald-400 mx-auto animate-pulse">
              <Zap className="w-7 h-7" />
            </div>
            <h4 className="font-bold text-slate-900 dark:text-white text-lg">AI Bot Deployed!</h4>
          ...
          <div className="space-y-4">
            {/* Top Stat Banner */}
            <div className="p-3.5 rounded-2xl bg-gradient-to-r from-purple-50 dark:from-purple-900/30 via-slate-50 dark:via-[#121626] to-cyan-50 dark:to-cyan-950/30 border border-purple-200 dark:border-purple-500/30 grid grid-cols-3 gap-2 text-center">
              <div>
                <div className="font-mono-num text-sm font-bold text-emerald-600 dark:text-emerald-400">89.4%</div>
              ...
              <div>
                <div className="font-mono-num text-sm font-bold text-amber-600 dark:text-amber-400">24/7</div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400">Uptime</div>
              </div>
            </div>

            {/* Select Strategy */}
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 block">Select AI Algorithm</label>
              <div className="space-y-2">
                {[
                  {
                    id: 'grid',
                  ...
                  <div
                    ...
                    className={`p-3 rounded-2xl border cursor-pointer transition-all ${
                      selectedStrategy === s.id
                        ? 'bg-purple-50 dark:bg-purple-600/15 border-purple-500 text-purple-950 dark:text-white shadow-sm'
                        : 'bg-slate-50 dark:bg-[#090C14] border-slate-200 dark:border-white/[0.06] text-slate-600 dark:text-slate-400 hover:border-purple-300 dark:hover:border-white/20'
                    }`}
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span className="font-bold text-xs text-slate-900 dark:text-white">{s.name}</span>
                      <span className="text-emerald-600 dark:text-emerald-400 font-mono-num font-bold text-xs">
                        {s.estApy}
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-snug mb-1.5">{s.desc}</p>
                    <div className="flex gap-2 text-[10px]">
                      <span className="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold">
                        {s.risk}
                      </span>
                      <span className="text-purple-600 dark:text-purple-400 font-medium">OKNexus Verified ✓</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-[#090C14] border border-slate-300 dark:border-white/10 focus-within:border-purple-500 transition-colors">
                <input
                  type="number"
                  value={allocation}
                ...
                />
                <span className="text-xs font-bold text-purple-600 dark:text-purple-400">USDT</span>
              </div>
            </div>
