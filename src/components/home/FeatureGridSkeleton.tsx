    ...
    >
      {items.map((item, index) => (
        <div
          ...
          className="flex items-center gap-3 p-3 rounded-2xl bg-[#0D101C] border border-white/[0.06] animate-shimmer shadow-sm"
        >
          {/* Icon box placeholder with subtle pulse & border */}
          <div className="w-9 h-9 rounded-xl bg-slate-800/80 border border-white/[0.06] flex-shrink-0 animate-pulse" />

          {/* Text lines */}
          <div className="min-w-0 flex-1">
            <div
              className={`h-3.5 rounded bg-slate-700/60 ${item.titleWidth} animate-pulse mb-1.5`}
            />
            <div
              className={`h-2.5 rounded bg-slate-800/60 ${item.descWidth} animate-pulse`}
            />
          </div>
        </div>
      ))}
