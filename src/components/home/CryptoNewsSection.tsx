...
export const CryptoNewsSection: React.FC<CryptoNewsSectionProps> = ({
  ...
  return (
    <section id="home-crypto-news" className="mb-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <h2 className="text-base font-bold text-white tracking-tight">Market Intelligence</h2>
              <span className="flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-bold tracking-wider uppercase bg-purple-500/20 text-purple-300 border border-purple-500/30">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 active:scale-95 border border-white/10 text-[11px] font-medium text-slate-300 hover:text-white transition-all disabled:opacity-50"
      ...
      <div className="p-3 mb-3 rounded-2xl bg-gradient-to-r from-[#121028] via-[#0D1527] to-[#120F24] border border-purple-500/20 shadow-sm">
        <div className="flex items-center justify-between text-xs mb-2">
          <div className="flex items-center gap-1.5 text-slate-300">
            <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
            <span className="font-semibold text-white">Your Favorite Assets</span>
            <span className="text-[11px] text-slate-400">• Top 5 Headlines</span>
          </div>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none pb-1 pt-0.5">
          <button
            ...
            className={`px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap transition-all flex items-center gap-1 ${
              selectedFilterSymbol === 'ALL'
                ...
                : 'bg-slate-800/70 text-slate-300 hover:bg-slate-700/70 hover:text-white border border-white/5'
            }`}
          >
          ...
          <button
            ...
            className="px-2 py-1 rounded-full text-[11px] font-medium text-purple-400 hover:text-purple-300 hover:bg-purple-500/10 transition-colors whitespace-nowrap ml-auto"
          >
            + Edit Favorites
          </button>
        </div>
        ...
        {searchQueries.length > 0 && (
          <div className="mt-2 pt-2 border-t border-white/[0.06] flex items-center justify-between text-[10px] text-slate-400">
            <div className="flex items-center gap-1 truncate max-w-[280px]">
              <Search className="w-2.5 h-2.5 text-purple-400 flex-shrink-0" />
              <span className="truncate">
                Query: <span className="text-slate-300 italic">{searchQueries[0]}</span>
              </span>
            </div>
            <span className="text-emerald-400/90 font-mono text-[9px] flex items-center gap-0.5 flex-shrink-0">
      ...
      {isLoading ? (
        <div className="space-y-2.5">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="p-3.5 rounded-2xl bg-[#0E131F]/90 border border-white/[0.06] animate-pulse"
            >
              ...
              <div className="w-full h-4 rounded bg-slate-800 mb-1.5" />
              <div className="w-3/4 h-3.5 rounded bg-slate-800/60" />
            </div>
          ))}
        </div>
      ) : headlines.length === 0 ? (
        <div className="p-6 rounded-2xl bg-[#0E131F]/70 border border-white/[0.06] text-center">
            ...
            return (
              <div
                ...
                className={`group rounded-2xl border transition-all cursor-pointer p-3.5 ${
                  isExpanded
                    ? 'bg-gradient-to-b from-[#141A29] to-[#0E131F] border-purple-500/40 shadow-md'
                    : 'bg-[#0D121D]/90 hover:bg-[#111726] border-white/[0.07] hover:border-white/15'
                }`}
              >
                {/* Meta Top Row: Asset Icon, Category, Sentiment Pill, Time */}
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-white/[0.06] border border-white/10 text-xs font-bold text-white">
                      <CoinIcon symbol={item.coinSymbol} size={16} />
                      <span>{item.coinSymbol}</span>
                    </div>

                    {item.category && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-purple-950/60 text-purple-300 border border-purple-500/30">
                        {item.category}
                      </span>
                    )}
                  ...
                  <div className="flex items-center gap-1.5 text-slate-400">
                    ...
                    <span className="text-[11px] text-slate-400 pl-1">{item.timeAgo}</span>
                  </div>
                </div>

                {/* Headline Title */}
                <h3 className="text-[13px] font-bold text-slate-100 group-hover:text-purple-300 transition-colors leading-snug mb-1.5">
                  {item.title}
                </h3>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        {item.url && (
                          <a
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 text-slate-200 hover:text-white text-xs font-semibold border border-white/10 transition-colors"
                          >
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="w-8 h-8 rounded-lg bg-slate-800/60 hover:bg-slate-700/80 border border-white/10 flex items-center justify-center text-slate-300 hover:text-white transition-colors"
                        ...
                        <button
                          ...
                          className={`w-8 h-8 rounded-lg border flex items-center justify-center transition-colors ${
                            isBookmarked
                              ...
                              : 'bg-slate-800/60 hover:bg-slate-700/80 border-white/10 text-slate-300 hover:text-white'
                          }`}
                ...
                <div className="flex items-center justify-between mt-2 pt-1.5 text-[11px] text-slate-400">
                  <div className="flex items-center gap-1.5">
                    ...
                    {item.sourceDomain && (
                      <span className="text-[10px] text-slate-400">({item.sourceDomain})</span>
                    )}
                  </div>

                  <div className="flex items-center gap-1 text-[11px] text-purple-400 hover:text-purple-300 font-medium">
