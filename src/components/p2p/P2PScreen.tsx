...
export const P2PScreen: React.FC<P2PScreenProps> = ({
  ...
  const handleOpenCreateAd = () => {
    setEditingAdId(null);
    setAdSide('sell');
    setAdCryptoSymbol('USDT');
    setAdPrice('1585.00');
    setAdAmount('1000');
    setAdMinLimit('10000');
    setAdMaxLimit('1500000');
    setAdPaymentMethod('Bank Transfer');
    setShowCreateAdModal(true);
  };

  const handleOpenEditAd = (ad: P2PAd) => {
    setEditingAdId(ad.id);
    setAdSide(ad.type);
    setAdCryptoSymbol((ad.cryptoSymbol as 'USDT' | 'BTC' | 'ETH') || 'USDT');
    setAdPrice(ad.price.toString());
    setAdAmount(ad.available.toString());
    setAdMinLimit(ad.minLimit.toString());
    setAdMaxLimit(ad.maxLimit.toString());
    setAdPaymentMethod(ad.paymentMethods?.[0] || 'Bank Transfer');
    setShowCreateAdModal(true);
  };
  ...
  return (
    <div id="p2p-full-screen" className="min-h-screen bg-white dark:bg-[#07090E] text-slate-900 dark:text-slate-100 flex flex-col transition-colors">
      ...
      {/* Main Body per Tab */}
      <main className="flex-1 pb-28 md:pb-12 pt-3 sm:pt-4 px-3 sm:px-6 lg:px-8 max-w-md md:max-w-4xl lg:max-w-7xl mx-auto w-full">
        {/* Mobile Unified P2P Top Tabs Bar */}
        <div className="flex md:hidden items-center justify-between gap-1 p-1 bg-slate-100 dark:bg-[#0E121E] rounded-2xl border border-slate-200/80 dark:border-white/[0.08] mb-3 shadow-2xs">
          {[
            { id: 'p2p_market', label: 'Market' },
            { id: 'p2p_orders', label: 'Orders' },
            { id: 'p2p_ads', label: 'My Ads' },
            { id: 'p2p_profile', label: 'Profile' },
          ].map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveP2PTab(item.id as P2PTab)}
              className={`flex-1 py-1.5 px-1 rounded-xl text-xs font-bold transition-all text-center ${
                activeP2PTab === item.id
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-white/[0.08]">
              <div>
                <h2 className="text-base font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                  <span>Merchant Advertisements</span>
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-500/25">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs active:scale-95 transition-all shadow-md shrink-0"
                >
              ...
              <div className="space-y-3.5">
                {localAds.map((ad) => {
                  ...
                  return (
                    <div
                      key={ad.id}
                      className={`p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-white dark:bg-[#0E141B] border transition-all relative overflow-hidden shadow-xs hover:shadow-md space-y-3.5 ${
                        isOnline
                          ? isBuy
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between gap-2 pt-1 flex-wrap">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-white/[0.03] text-slate-500 dark:text-slate-400 border border-slate-200/60 dark:border-white/5">
                            15 min window
                          </span>
                        </div>
                      ...
                      {/* Hero Unit Price Box */}
                      <div className="p-3 sm:p-3.5 rounded-2xl bg-slate-50 dark:bg-[#070A12] border border-slate-200/80 dark:border-white/[0.05] flex items-center justify-between">
                        <div>
                      ...
                      {/* Footer Actions */}
                      <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-white/[0.06] text-xs flex-wrap gap-2">
                        {/* Copy Ad ID */}
                        <button
                          type="button"
                          onClick={() => {
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold text-purple-600 dark:text-purple-400 bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 transition-all active:scale-95"
                          >
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              onShowToast?.('Ad Deleted', `Advertisement #${ad.id.slice(-6).toUpperCase()} removed.`, 'alert');
                            }}
        ...
        {activeP2PTab === 'p2p_profile' && (
          <div className="space-y-4">
            ...
            {/* Upgraded P2P Profile Options Vertical List (Directly Below Profile Card) */}
            <div className="rounded-3xl bg-white dark:bg-[#0E141B] border border-slate-200 dark:border-white/[0.08] divide-y divide-slate-100 dark:divide-white/[0.05] shadow-xs overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                      <span>My Trade Orders</span>
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-purple-500/15 text-purple-700 dark:text-purple-300">
                        {localOrders.length} ORDERS
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">View all active escrow orders and transaction history</div>
                  </div>
                </div>
                <ChevronRight className="w-4 h-4 text-slate-400 group-hover:translate-x-0.5 transition-transform shrink-0 ml-2" />
              </button>
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-indigo-500/15 text-indigo-700 dark:text-indigo-300">
                        {localAds.length} ADS
                      </span>
                    </div>
              ...
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                      <span>Trust Score & Reputation Breakdown</span>
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-cyan-500/15 text-cyan-700 dark:text-cyan-400 border border-cyan-500/25">
                        97/100 ELITE
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Gauge analytics, dispute-free trade record, release metrics</div>
                  </div>
                </div>
                <ChevronRight className="w-4 h-4 text-slate-400 group-hover:translate-x-0.5 transition-transform shrink-0 ml-2" />
              </button>
      ...
      {/* Post / Edit Ad Modal (Production Seller Flow) */}
      {showCreateAdModal && (
          ...
          <div className="w-full max-w-lg bg-[#0F1322] border border-white/10 rounded-3xl p-6 space-y-4 animate-fade-in text-xs shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-white/[0.06]">
              <div>
                <h3 className="font-extrabold text-base text-white">
                  {editingAdId ? 'Edit P2P Advertisement' : 'Create P2P Merchant Advertisement'}
                </h3>
                <span className="text-[10px] text-purple-400 font-semibold">
                  {editingAdId ? `Editing Ad #${editingAdId.slice(-6).toUpperCase()} • Liquidity Desk` : 'Seller & Liquidity Provider Protocol'}
                </span>
              </div>
              <button
                onClick={() => {
            ...
            <div className="space-y-3">
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    if (editingAdId) {
                        ...
                        prev.map((item) =>
                          item.id === editingAdId
                            ? {
                                ...
                                price: parseFloat(adPrice) || item.price,
                                available: parseFloat(adAmount) || item.available,
                                minLimit: parseFloat(adMinLimit) || item.minLimit,
                                maxLimit: parseFloat(adMaxLimit) || item.maxLimit,
                      ...
                      setShowCreateAdModal(false);
                      setEditingAdId(null);
                      onShowToast?.('Ad Updated', 'Advertisement parameters saved successfully.', 'success');
                    } else {
                ...
                >
                  {editingAdId ? 'Save Ad Changes →' : 'Publish Advertisement →'}
                </button>
              </div>
            </div>
          </div>
