...
export const MarketsScreen: React.FC<MarketsScreenProps> = ({
  ...
  return (
    <div
      id="markets-screen"
      className={`pb-28 md:pb-12 pt-3 px-3 sm:px-6 lg:px-8 mx-auto min-h-screen text-slate-100 transition-all duration-300 ${
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 gap-2">
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold font-display text-white tracking-tight">Markets</h1>
          
          {/* List vs Heatmap Segmented View Switcher */}
          <div className="flex items-center bg-[#111624] border border-white/10 rounded-xl p-0.5">
            <button
        ...
        <div className="flex items-center gap-1.5 flex-1 max-w-[200px] sm:max-w-[260px] justify-end">
          <div className="relative flex-1">
            <input
              ...
              className="w-full h-8 pl-8 pr-3 text-xs rounded-xl bg-[#111624] border border-white/10 text-white placeholder-slate-400 focus:outline-none focus:border-purple-500/50"
            />
          ...
          {onOpenPriceAlerts && (
            <button
              ...
              className="p-1.5 rounded-xl bg-[#111624] border border-white/10 hover:border-purple-500/40 text-slate-400 hover:text-purple-300 transition-colors flex-shrink-0"
              title="Price Alerts"
      ...
      <div className="flex items-center justify-between border-b border-white/[0.08] pb-2 mb-3 text-xs font-semibold">
      ...
      <div className="flex items-center justify-between gap-2 mb-3">
        ...
        <button
          ...
          className="text-[11px] font-semibold text-purple-400 hover:text-purple-300 flex items-center gap-1 flex-shrink-0 transition-colors"
        >
          {viewMode === 'list' ? (
            <>
      ...
      {viewMode === 'heatmap' ? (
        ...
        <div className="animate-in fade-in duration-200">
          ...
          <div
            ...
            onClick={() => setViewMode('heatmap')}
            className="p-2.5 rounded-xl border border-purple-500/25 bg-gradient-to-r from-purple-900/15 via-[#0A1020] to-[#0A1020] hover:border-purple-500/45 transition-all cursor-pointer flex items-center justify-between group mb-3 shadow-sm"
            title="Open Market Heatmap"
          >
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-rose-500/30 via-purple-500/30 to-emerald-500/30 flex items-center justify-center border border-white/10 group-hover:scale-105 transition-transform">
            ...
            <div className="flex items-center gap-1 text-[11px] font-semibold text-purple-400 group-hover:text-purple-300">
          ...
          {/* Column Headers */}
          <div className="flex items-center justify-between px-3 py-2 text-[11px] font-semibold text-slate-400 border-b border-white/[0.08] mb-1">
            <button
              onClick={() => handleSort('pair')}
              className="flex items-center gap-1.5 hover:text-slate-200 transition-colors w-44 sm:w-56"
            >
              <span>Pair / Name</span>
            ...
            <div className="flex items-center justify-end gap-5 lg:gap-8 flex-1">
              ...
              <div className="hidden sm:block text-center w-24">
                <span className="text-slate-400">7d Trend</span>
              </div>
              <button
                onClick={() => handleSort('price')}
                className="flex items-center justify-end gap-1 hover:text-slate-200 transition-colors w-24 text-right"
              >
                <span>Price</span>
                <ArrowUpDown className="w-3 h-3" />
              </button>
              ...
              <button
                ...
                className="flex items-center justify-end gap-1 hover:text-slate-200 transition-colors w-20 text-right"
              >
            ...
            {filteredPairs.length === 0 ? (
              ...
              filteredPairs.map((pair) => (
                <div
                  ...
                  className="flex items-center justify-between p-2.5 sm:p-3 rounded-xl hover:bg-white/[0.04] active:bg-white/[0.08] transition-colors cursor-pointer group"
                >
                  ...
                  <div className="flex items-center gap-2.5 w-44 sm:w-56">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-slate-400 truncate max-w-[90px] sm:max-w-[130px]">
                        {pair.name}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-5 lg:gap-8 flex-1">
                    ...
                    <div className="hidden md:block text-right w-24 font-mono-num text-xs text-slate-300">
                      ${pair.high24h >= 1 ? pair.high24h.toLocaleString('en-US', { minimumFractionDigits: 2 }) : pair.high24h.toFixed(4)}
                    </div>

                    {/* 24h Low (Tablet & Desktop) */}
                    <div className="hidden md:block text-right w-24 font-mono-num text-xs text-slate-400">
                      ${pair.low24h >= 1 ? pair.low24h.toLocaleString('en-US', { minimumFractionDigits: 2 }) : pair.low24h.toFixed(4)}
                    </div>

                    {/* 24h Volume (Desktop) */}
                    <div className="hidden lg:block text-right w-24 font-mono-num text-xs text-slate-400">
                    ...
                    <div className="text-right w-24">
                      ...
                      <div className="text-[10px] font-mono-num text-slate-400 md:hidden">
                    ...
                    <div className="hidden md:block w-16 text-right">
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-purple-600/20 hover:bg-purple-600 border border-purple-500/40 text-purple-300 hover:text-white transition-all shadow-xs"
                      >
                        Trade
                      </button>
                    </div>
