...
export const OrdersHistoryPanel: React.FC<OrdersHistoryPanelProps> = ({
  ...
  const renderStatusBadge = (order: OpenOrder) => {
    ...
    switch (order.status) {
      case 'open':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wide bg-amber-500/15 text-amber-300 border border-amber-500/30">
      ...
      case 'partial':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wide bg-purple-500/15 text-purple-300 border border-purple-500/30">
      ...
      case 'completed':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wide bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">
      ...
      case 'cancelled':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wide bg-slate-500/15 text-slate-400 border border-slate-500/30">
    ...
    >
        ...
        <div className="flex items-center gap-2 sm:gap-3 text-xs font-semibold overflow-x-auto no-scrollbar py-0.5">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 pb-1 relative whitespace-nowrap transition-colors ${
              tab === 'orders' ? 'text-white font-bold' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <span>Orders</span>
            <span
              className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono-num font-bold ${
                allOrders.length > 0 ? 'bg-purple-600/40 text-purple-200' : 'bg-white/[0.06] text-slate-400'
              }`}
            >
              {allOrders.length}
            </span>
          ...
          <button
            ...
            onClick={() => setTab('open')}
            className={`flex items-center gap-1.5 pb-1 relative whitespace-nowrap transition-colors ${
              tab === 'open' ? 'text-white font-bold' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <span>Open</span>
            <span
              className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono-num font-bold ${
                openCount > 0 ? 'bg-amber-500/20 text-amber-300' : 'bg-white/[0.06] text-slate-400'
              }`}
            >
              {openCount}
            </span>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 pb-1 relative whitespace-nowrap transition-colors ${
              tab === 'history' ? 'text-white font-bold' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <span>History</span>
            <span className="px-1.5 py-0.2 rounded-full text-[10px] font-mono-num font-bold bg-white/[0.06] text-slate-400">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 pb-1 relative whitespace-nowrap transition-colors ${
              tab === 'assets' ? 'text-white font-bold' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Wallet className="w-3.5 h-3.5 text-amber-400" />
            <span>Assets</span>
            {tab === 'assets' && (
              <span className="absolute -bottom-2 left-0 right-0 h-[2px] bg-amber-400 rounded-full" />
            )}
          </button>
        </div>
      ...
      {tab === 'orders' && (
        <div className="space-y-2.5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-1 border-b border-white/[0.04]">
            ...
            <div className="flex items-center gap-1 text-[10px] text-slate-400 overflow-x-auto no-scrollbar">
          ...
          {processedOrders.length === 0 ? (
            <div className="py-8 px-4 text-center rounded-xl bg-[#06080E] border border-white/[0.04]">
              <Clock className="w-6 h-6 text-slate-600 mx-auto mb-1.5" />
              <p className="text-xs text-slate-300 font-semibold">No orders found</p>
            ...
            <div className="space-y-2">
              {processedOrders.map((ord) => {
                ...
                return (
                  <div
                    key={ord.id}
                    className="p-2.5 sm:p-3 rounded-xl bg-[#06080E] border border-white/[0.06] hover:border-white/10 transition-all text-xs"
                  >
                    ...
                    <div className="flex items-center justify-between gap-1.5 mb-1.5">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className="font-bold text-white tracking-tight">{ord.symbol}</span>

                        <span className="text-[10px] text-slate-400 uppercase font-mono-num px-1 py-0.2 rounded bg-white/[0.04]">
                          {ord.type}
                        </span>
                      ...
                      <div className="flex items-center gap-1 text-[10px] text-slate-400 font-mono-num shrink-0">
                        <span>{ord.time}</span>
                        <button
                          type="button"
                    ...
                    <div className="grid grid-cols-3 gap-2 text-[11px] font-mono-num text-slate-300 pt-0.5">
                      <div>
                        <span className="text-slate-500 text-[9px] uppercase tracking-wider block">Price</span>
                        <span className="font-semibold text-white">${ord.price.toFixed(2)}</span>
                      </div>
                    ...
                    {/* Fill Progress Bar */}
                    <div className="w-full bg-slate-800/80 rounded-full h-1 overflow-hidden mt-2">
                      <div
                    ...
                    <div className="flex items-center justify-between mt-2 pt-1 border-t border-white/[0.04] text-[10px]">
                      ...
                      {isOpen ? (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-md bg-rose-500/15 border border-rose-500/30 text-rose-300 text-[11px] font-semibold hover:bg-rose-500/25 active:scale-95 transition-all"
                        >
                          Cancel Order
                        </button>
                      ) : ord.status === 'completed' ? (
                    ...
                    {isExpanded && (
                      <div className="mt-2.5 pt-2 border-t border-white/[0.06] grid grid-cols-2 gap-2 text-[10px] font-mono-num text-slate-400 bg-white/[0.02] p-2 rounded-lg">
                        <div>
                        ...
                        <div>
                          ...
                          <span className="text-slate-200 font-semibold">{fillPct.toFixed(1)}%</span>
                        </div>
                      </div>
                    )}
                  </div>
      ...
      {tab === 'open' && (
        <div className="space-y-2">
          {openOrders.length === 0 ? (
            <div className="py-6 px-4 text-center rounded-xl bg-[#06080E] border border-white/[0.04]">
              <Clock className="w-6 h-6 text-slate-600 mx-auto mb-1.5" />
              <p className="text-xs text-slate-400 font-medium">No open orders for {pair.symbol}</p>
            ...
            openOrders.map((ord) => {
              ...
              return (
                <div
                  key={ord.id}
                  className="p-3 rounded-xl bg-[#06080E] border border-white/[0.06] flex items-center justify-between text-xs"
                >
                  <div className="space-y-1 flex-1 pr-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {renderStatusBadge(ord)}
                      <span className="text-[10px] text-slate-400 ml-auto font-mono-num">{ord.time}</span>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-[11px] font-mono-num text-slate-300">
                      <div>
                  ...
                  <button
                    type="button"
                    onClick={() => onCancelOrder(ord.id)}
                    className="px-2.5 py-1 rounded-lg bg-rose-500/15 border border-rose-500/30 text-rose-400 text-[11px] font-semibold hover:bg-rose-500/25 active:scale-95 transition-all"
                  >
                    Cancel
                  </button>
                </div>
        ...
        <div className="space-y-2">
          {historyOrders.length === 0 ? (
            <div className="py-6 px-4 text-center rounded-xl bg-[#06080E] border border-white/[0.04]">
              ...
              <p className="text-xs text-slate-400 font-medium">No order history yet</p>
            </div>
          ) : (
            historyOrders.map((ord) => (
              <div
                key={ord.id}
                className="p-2.5 rounded-xl bg-[#06080E] border border-white/[0.06] text-xs font-mono-num"
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold text-white">{ord.symbol}</span>
                    <span className="text-[10px] text-slate-400 capitalize">({ord.type})</span>
                  </div>
                  {renderStatusBadge(ord)}
                </div>
                <div className="flex items-center justify-between text-[11px] text-slate-300">
                  <span>Price: ${ord.price.toFixed(2)}</span>
                  <span>
                    Amount: {ord.amount} {pair.base}
                  </span>
                </div>
      ...
      {tab === 'assets' && (
        <div className="grid grid-cols-2 gap-2.5">
          <div className="p-3 rounded-xl bg-[#06080E] border border-white/[0.06]">
            <span className="text-[10px] text-slate-400 font-medium block mb-0.5">{pair.base} Available</span>
            <div className="font-mono-num font-bold text-base text-white">{availableCrypto.toFixed(4)}</div>
            <span className="text-[10px] font-mono-num text-slate-400">≈ ${cryptoUsdValue.toFixed(2)} USD</span>
          </div>

          <div className="p-3 rounded-xl bg-[#06080E] border border-white/[0.06]">
            <span className="text-[10px] text-slate-400 font-medium block mb-0.5">USDT Available</span>
            ...
            <span className="text-[10px] font-mono-num text-slate-400">Total Liquidity</span>
          </div>
        </div>
      )}
    </div>
