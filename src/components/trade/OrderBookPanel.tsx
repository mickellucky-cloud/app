  ...
  return (
    ...
    >
      ...
      {!isCompact ? (
        ...
        <div className="flex items-center justify-between pb-1.5 mb-1.5 border-b border-slate-200 dark:border-white/[0.06]">
          <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300">Order Book</span>
          <div className="flex items-center gap-1">
            <span className="text-[9px] text-slate-400 font-mono-num">0.1</span>
          </div>
        </div>
      )}

      {activeTab === 'book' || isCompact ? (
        <div>
          {/* Table Header */}
          <div className={`grid ${isCompact ? 'grid-cols-2' : 'grid-cols-3'} text-[10px] text-slate-500 dark:text-slate-400 font-medium pb-1 mb-1 border-b border-slate-200 dark:border-white/[0.04]`}>
            <span>Price</span>
            <span className="text-right">Qty</span>
            {!isCompact && <span className="text-right">Total</span>}
          </div>
          ...
          <div className="space-y-0.5">
            {asks.slice(0, 4).map((ask, i) => (
              <button
                ...
                className={`w-full grid ${isCompact ? 'grid-cols-2' : 'grid-cols-3'} text-[10px] sm:text-[11px] font-mono-num py-0.5 px-1 rounded relative text-left transition-all active:scale-[0.99] ${
          ...
          <div className="my-1.5 py-1 px-1.5 rounded-lg bg-slate-100 dark:bg-[#111624] border border-slate-200 dark:border-white/[0.06] flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <span
                className={`font-mono-num font-extrabold text-xs sm:text-sm ${
            ...
            <div className="text-[9px] font-mono-num text-slate-500 dark:text-slate-400">
              <span className={pair.change24h >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}>
          ...
          <div className="mt-2 pt-1.5 border-t border-slate-200/60 dark:border-white/[0.04]">
            <div className="flex justify-between text-[9px] font-mono-num font-bold mb-1">
              ...
              <span className="text-rose-600 dark:text-rose-400">42% S</span>
            </div>
            <div className="h-1 w-full rounded-full bg-slate-200 dark:bg-slate-800 flex overflow-hidden">
              <div
